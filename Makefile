...
install-curljob:
	...
	echo "pw=\$$(docker exec -t hoardercache cat /etc/apt-cacher-ng/security.conf | sed 's|AdminAuth: acng:||' | tr -d '\n')" | sudo tee -a /usr/local/bin/scheduled-cacher-import
	echo "echo \"$pw\"" | sudo tee -a /usr/local/bin/scheduled-cacher-import
	echo "curl \\" | sudo tee -a /usr/local/bin/scheduled-cacher-import
	echo "  -u acng:\"\$$(docker exec -t hoardercache cat /etc/apt-cacher-ng/security.conf | sed 's|AdminAuth: acng:||' | tr -d '\n')\" \\" | sudo tee -a /usr/local/bin/scheduled-cacher-import
